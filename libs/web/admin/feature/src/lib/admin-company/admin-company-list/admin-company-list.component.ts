...
import { AdminCompanyListStore } from './admin-company-list.store'

@Component({
  template: `
    <ng-container *ngIf="vm$ | async as vm">
      <ui-page-header title="Companys" linkPath="create" linkTitle="Create Company"></ui-page-header>
      <ng-container *ngIf="vm.loading">
        <div class="flex py-36 animate-pulse justify-center align-center">LOADING...</div>
      </ng-container>
      <ng-container *ngIf="!vm.loading">
        <ng-container *ngIf="!vm.items?.length">
          <div class="flex py-20 justify-center align-center">No Companys found...</div>
        </ng-container>
  ...
  `,
  providers: [AdminCompanyListStore],
})
export class AdminCompanyListComponent implements OnInit {
  readonly vm$ = this.store.vm$
  constructor(private readonly store: AdminCompanyListStore) {}

  ngOnInit(): void {
    this.store.loadCompanysEffect()
  }
}
